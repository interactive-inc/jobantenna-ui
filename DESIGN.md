...
転職・求人サービスのための、製品から独立した規範デザインシステム。トークンの正はこの YAML、実装の正は jobantenna-ui の shadcn レジストリ、思想と用途の解説はガイドラインサイトにある。参照は常に「製品 → デザインシステム」の一方向で、製品側でトークンや原則を上書きしない。

コンセプトは6つ。(1) スマホの片手で完結する — 設計は常にモバイルファースト。1カラム、主要アクションは親指が届く下側、タップ領域は最小 36px。(2) 重い決断を軽い一歩に分解する — 応募のような重い行動の隣に必ず軽い行動(保存・気になる)を併置し、空状態と完了画面では次の一歩を1つ示す。(3) モダンでポップに、でもポップ過ぎない — ポップさは形と一色のアクセントと励ますことばの3つだけで出し、土台はグレースケールの落ち着きで真剣さを支える。(4) 信頼は細部でつくる — 公開範囲は行動の前に明示し、破壊的操作は必ず確認を挟む。(5) 地域の言葉で話す — 地名・方言・土地の文脈で話しかけ、文言は地域で差し替え可能に設計する。(6) 両側で同じ言語 — 探す人の画面は余裕と励ましに、企業の管理画面は密度と効率に、同じトークンとコンポーネントの振れ幅で応える。
...
- コンポーネントは jobantenna-ui レジストリのものだけを使い、独自実装で複製しない。
- トークンの変更はこのファイルで行い、`npx @google/design.md lint` を通してから配布する。製品側で上書きしない。
